package no.imr.nmdapi.nmdcruiseseries.security.access.voters;
 ...
 */
@Service
public class CruiseseriesAccessDecisionVoter implements AccessDecisionVoter<FilterInvocation> {
    ...
    public int vote(Authentication auth, FilterInvocation obj, Collection<ConfigAttribute> confAttrs) {
        if (obj.getFullRequestUrl().contains(CruiseseriesController.CRUISESERIES_URL)) {
            if (obj.getHttpRequest().getMethod().equalsIgnoreCase(HttpMethod.GET.name()) || obj.getHttpRequest().getMethod().equalsIgnoreCase(HttpMethod.HEAD.name())) {
                ...
                // Write access.
                if (auth.isAuthenticated() && auth.getAuthorities().contains(new SimpleGrantedAuthority(CruiseseriesAccessDecisionVoter.ROLE_WRITE_ACCESS))) {
                    return ACCESS_GRANTED;
                } else {
